...
const SpellingAdventure: React.FC = () => {
  ...
  const initPool = useCallback(
    (width: number, height: number, word: string = "") => {
      ...
      const perRow = 7; // Fewer letters per row to center the pool better
      const rows: number[] = [];
      ...
      // Start slightly higher up, more centered vertically between tray and bottom
      // Place tiles in the space BELOW the tray, centred vertically in that space.
      // trayBottom = TRAY_Y + 50 (tray renders from TRAY_Y-50 to TRAY_Y+50)
      const trayBottom = TRAY_Y + 50;
      const availableSpace = height - trayBottom;
      // Minimum offset keeps at least ~0.85 tile-height gap from the tray
  ...
  useEffect(() => {
    ...
    const onResults = (results: any) => {
      ...
      // Hard recovery: if canvas is still 0×0 (e.g. clientWidth was 0 at mount),
      // fix it now using the actual container size before drawing anything.
      ...
      if (handPos && isPinching && !isCorrectRef.current) {
        if (!activeTileId.current) {
          ...
          if (tile) {
            // Tile follows index tip directly
            tile.x = handPos!.x;
            tile.y = handPos!.y;
          }
        }
